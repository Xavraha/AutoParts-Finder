    ...
    <div className="flex flex-col items-center gap-10 py-10">
      ...
      <div className="text-center">
        <h1 className="text-4xl font-extrabold tracking-tight text-gray-900 sm:text-5xl">
          ...
          <span className="text-blue-600"> Across the US</span>
        </h1>
      ...
      {/* Search card */}
      <div className="w-full max-w-2xl rounded-2xl bg-white p-6 shadow-lg ring-1 ring-gray-200">
        {/* Mode toggle */}
        <div className="mb-4 flex rounded-lg bg-gray-100 p-1">
          <button
            ...
            className={`flex-1 rounded-md py-1.5 text-sm font-medium transition-colors ${
              searchMode === 'text' ? 'bg-white shadow text-gray-900' : 'text-gray-500 hover:text-gray-700'
            }`}
          >
            Search by part name
          </button>
          <button
            ...
            className={`flex-1 rounded-md py-1.5 text-sm font-medium transition-colors ${
              searchMode === 'vin' ? 'bg-white shadow text-gray-900' : 'text-gray-500 hover:text-gray-700'
            }`}
          >
            Search by VIN
          </button>
      ...
      <div className="w-full max-w-2xl">
        <p className="mb-3 text-center text-sm font-medium text-gray-500">Popular searches</p>
        <div className="flex flex-wrap justify-center gap-2">
          {POPULAR_SEARCHES.slice(0, 6).map((s) => (
            <Link
              key={s.slug}
              href={`/parts/${s.slug}`}
              className="rounded-full border border-gray-200 bg-white px-3 py-1.5 text-xs text-gray-600 shadow-sm hover:border-blue-300 hover:text-blue-600 transition-colors"
            >
      ...
      <div className="flex flex-col items-center gap-3">
        <p className="text-sm text-gray-400">Searching across</p>
        <div className="flex flex-wrap justify-center gap-3">
          {SOURCES.map((s) => (
            <div key={s.name} className="flex items-center gap-2 rounded-full bg-white px-4 py-2 shadow-sm ring-1 ring-gray-200">
