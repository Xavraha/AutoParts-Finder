...
export function FilterPanel() {
  ...
  return (
    <aside className="flex flex-col gap-5 rounded-xl border border-gray-200 bg-white p-4 shadow-sm">
      {/* Sort */}
      <div>
        <h3 className="mb-2 text-sm font-semibold text-gray-700">Sort by</h3>
        <select
          value={sortBy}
      ...
      {/* Condition */}
      <div>
        <h3 className="mb-2 text-sm font-semibold text-gray-700">Condition</h3>
      ...
      <div>
        ...
        <div className="flex flex-col gap-1.5">
          {SOURCES.map((s) => (
            <label key={s.key} className="flex cursor-pointer items-center gap-2 text-sm text-gray-600">
              <input
                type="checkbox"
                defaultChecked
                className="rounded border-gray-300 text-blue-600"
